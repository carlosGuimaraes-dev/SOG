...
- Run PR creation/update through `gh` CLI only. In daemon/non-interactive
  Symphony runs, do not use GitHub MCP connector PR tools; they can ask for
  approval/elicitation and cause `turn_input_required` retry loops.
...
5. Ensure a PR exists for the branch:
   ...
   - Use `gh pr create` / `gh pr edit`; do not substitute GitHub MCP connector
     PR tools in non-interactive runs.
